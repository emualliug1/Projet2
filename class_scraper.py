...
class Scraper(object):
    ...
    def recuperer_informations_livre(self):
        ...
        chercher_balise_td_html = self.soup('td')
    ...
    def recuperer_urls_images(self):
        ...
        for i in self.soup('img'):
            url_image = (i['src'])
            self.images_urls.append(URL_MAITRE + url_image[URL_IMAGE_DEBUT:])
    ...
    def changer_page(self, nouvelle_url):
        ...
        while debut_iteration <= max_page:
            h = nouvelle_url
            self.iteration_url.append(h[:ITERATION_FIN] + 'page-' + str(debut_iteration) + '.html')
            debut_iteration += 1
